...
contract PrivateSale is Ownable {
    ...
    //user function-------------------------------------------------------------
    function deposit(uint256 amount, address token) isUsedToken(token) unfinishedRound external {
        ...
        require(rounds[countRound-1].minimumSaleAmount <= amount*10**(18-AggregatorV3Interface(token).decimals()), "Minimum sale amount more than your amount");
        require(rounds[countRound-1].maximumSaleAmount == 0||rounds[countRound-1].maximumSaleAmount>=amount*10**(18-AggregatorV3Interface(token).decimals()), " Your amount more than maximum sale amount");
        require(rounds[countRound-1].endTimestamp>block.timestamp||rounds[countRound-1].endTimestamp==0, "Round is ended, round time expired");
        require(rounds[countRound-1].tokenRate==0||rounds[countRound-1].sumTokens==0||rounds[countRound-1].sumTokens>=(rounds[countRound-1].totalReserve+amount*10**(18-AggregatorV3Interface(token).decimals()))*(1 ether)/rounds[countRound-1].tokenRate, "Round is ended, all tokens sold");
        require(rounds[countRound-1].maxMoney==0||rounds[countRound-1].maxMoney>=(rounds[countRound-1].totalReserve+amount*10**(18-AggregatorV3Interface(token).decimals())), "The round is over, the maximum required value has been reached, or your amount is greater than specified in the conditions of the round");
        ...
        rounds[countRound-1].vestingContract.deposit(msg.sender, token, amount*10**(18-AggregatorV3Interface(token).decimals()));
    }
    ...
    function returnDeposit(uint256 round) external {
        ...
        IERC20(rounds[round].depositToken[msg.sender]).safeTransfer(msg.sender, rounds[round].sums[msg.sender]/10**(18-AggregatorV3Interface(rounds[round].depositToken[msg.sender]).decimals()));
    ...
    //owner functions
    function addToken(address _token,address _oracles) onlyOwner external {
    ...
    function setInvestorWallet(address _investorWallet) onlyOwner finishedRound external {
        investorWallet = _investorWallet;
    }

    function setExpenseAddress(address _expenseAddress) onlyOwner finishedRound external {
    ...
    function setBLID(address _BLID) onlyOwner external {
        require(BLID==address(0), "BLID was set");
        BLID = _BLID;
    }
 
    function newRound(InputNewRound memory input) onlyOwner finishedRound external {
        ...
        require(input._endTimestamp==0||input._endTimestamp>block.timestamp, "_endTimestamp must be unset or more than now timestamp");
        if(input._typeRound == 1){
            require(input._tokenRate>0, "Need set _tokenRate and _tokenRate must be more than 0");
            require(IERC20(BLID).balanceOf(address(this))>=input._sumTokens, "_sumTokens more than this smart contract have BLID");
            ...
            rounds[countRound].vestingContract = new TokenVestingGroup(BLID, input._duration, input._durationCount, inputTokens);
            countRound++;
    ...
    function setRateToken(uint256 rate) onlyOwner unfinishedRound external {
        require(rounds[countRound-1].typeRound==1, "This round auto generate rate");
        rounds[countRound-1].tokenRate = rate;
    }

    function setEndTimestamp(uint256 _endTimestamp) onlyOwner unfinishedRound external {
        rounds[countRound-1].endTimestamp = _endTimestamp;
    }

    function setSumTokens(uint256 _sumTokens) onlyOwner unfinishedRound external {
        require(IERC20(BLID).balanceOf(address(this))>=_sumTokens, "_sumTokens more than this smart contract have BLID");
    ...
    function setStartTimestamp(uint256 _startTimestamp) onlyOwner unfinishedRound external {
        require(block.timestamp<_startTimestamp, "Round has been started");
        rounds[countRound-1].startTimestamp = _startTimestamp;
    }

    function setMaxMoney(uint256 _maxMoney) onlyOwner unfinishedRound external {
    ...
    function addWhiteList(address account) onlyOwner unfinishedRound external {
        rounds[countRound-1].whiteList[account] = true;
    }
    
    function addWhiteListByArray(address[] calldata accounts) onlyOwner unfinishedRound external {
        for(uint256 i = 0 ; i< accounts.length;i++){
            rounds[countRound-1].whiteList[accounts[i]] = true;
        }
    }

    function deleteWhiteList(address account) onlyOwner unfinishedRound external {
        rounds[countRound-1].whiteList[account] = false;
    }
    
    function deleteWhiteListByArray(address[] calldata accounts) onlyOwner unfinishedRound external {
        for(uint256 i = 0 ; i< accounts.length;i++){
            rounds[countRound-1].whiteList[accounts[i]] = false;
        }
    }

    function finishRound() onlyOwner external {
        require(countRound!=0&&!rounds[countRound-1].finished, "Last round has been finished");
        uint256[] memory rates = new uint256[](countTokens);
        uint256 sumUSD = 0;
        for(uint256  i = 0; i < countTokens; i++){
            if(rounds[countRound-1].tokenReserve[tokens[i]]==0) continue;
            IERC20(tokens[i]).safeTransfer(expenseAddress,rounds[countRound-1].tokenReserve[tokens[i]]/10**(18-AggregatorV3Interface(tokens[i]).decimals())-rounds[countRound-1].tokenReserve[tokens[i]]/10**(18-AggregatorV3Interface(tokens[i]).decimals())*(rounds[countRound-1].percentOnInvestorWallet)/100);
            IERC20(tokens[i]).safeTransfer(investorWallet,rounds[countRound-1].tokenReserve[tokens[i]]/10**(18-AggregatorV3Interface(tokens[i]).decimals())*(rounds[countRound-1].percentOnInvestorWallet)/100);
            rates[i] = (uint256(AggregatorV3Interface(oracles[tokens[i]]).latestAnswer())*10**(18-AggregatorV3Interface(oracles[tokens[i]]).decimals()));
            
            sumUSD += rounds[countRound-1].tokenReserve[tokens[i]]*rates[i]/(1 ether);
            if(rounds[countRound-1].typeRound==1) rates[i] = rates[i]*(1 ether)/rounds[countRound-1].tokenRate;
            if(rounds[countRound-1].typeRound==2) rates[i] = rounds[countRound-1].sumTokens*rates[i]/sumUSD;
        }
        if(sumUSD != 0){
            rounds[countRound-1].vestingContract.finishRound(block.timestamp+rounds[countRound-1].lockup, rates);
            if(rounds[countRound-1].typeRound==1) IERC20(BLID).safeTransfer(address(rounds[countRound-1].vestingContract), sumUSD*(1 ether)/rounds[countRound-1].tokenRate);
        }
        if(rounds[countRound-1].typeRound==2) IERC20(BLID).safeTransfer(address(rounds[countRound-1].vestingContract), rounds[countRound-1].sumTokens);
        if(rounds[countRound-1].burnable && rounds[countRound-1].sumTokens-sumUSD * (1 ether)/rounds[countRound-1].tokenRate!=0){
            IBurnable(BLID).burn(rounds[countRound-1].sumTokens - sumUSD*(1 ether)/rounds[countRound-1].tokenRate);
        }
        rounds[countRound-1].finished = true;
    }

    function cancelRound() onlyOwner external {
        require(countRound!=0&&!rounds[countRound-1].finished, "Last round has been finished");
        rounds[countRound-1].finished = true;
    ...
    function getLockedTokens(uint256 id) view public returns (uint256) {
        if(rounds[id].tokenRate==0) return 0;
        return(rounds[id].totalReserve*(1 ether)/rounds[id].tokenRate);
    }

    function getRoundDynamicInfromation(uint256 id) view public returns (uint256, uint256, bool) {
