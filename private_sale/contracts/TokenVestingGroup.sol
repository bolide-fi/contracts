...
contract TokenVestingGroup is Ownable  {
    ...
    function deposit(address user, address token, uint256 amount) onlyOwner external {
        _userToken[user] = token;
        _sumUser[user] = amount;
    }

    function finishRound(uint256 startTimestampValue, uint256[] memory tokenRate) onlyOwner external {
    ...
    */
    ...
    function returnDeposit(address user) onlyOwner external {
        require(_startTimestamp==0, "Vesting has been started");
