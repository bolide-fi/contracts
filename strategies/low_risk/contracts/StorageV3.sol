...
contract StorageV3 is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    event SetBoostingInfo(uint256 maxBlidPerUSD, uint256 blidPerBlock, uint256 maxActiveBLID);
    ...
    event SetOracleLatestAnswer(address token, int256 latestAnswer, uint256 timestamp);
    event SetAdmin(address admin);
    event UpgradeVersion(string version, string purpose);
    event SetLeaveTokenPolicy(uint256 limit, uint256 leavePerentage, uint256 leaveFixed);
     ...
     */
    function upgradeVersion(string memory version, string memory purpose) external onlyOwner {
    ...
    function addToken(address _token, address _oracles) external onlyOwner {
        ...
        require(
            _token == ZERO_ADDRESS ||
                (_token != ZERO_ADDRESS && IERC20MetadataUpgradeable(_token).decimals() <= 18),
            "E17"
        );
        ...
        (, int256 latestAnswer, , , ) = AggregatorV3Interface(_oracles).latestRoundData();
        OracleLatestAnswerInfo storage _oracleLatestAnswerInfo = oracleLatestAnswerInfo[_token];
     ...
     */
    function setOracleDeviationLimit(uint256 _oracleDeviationLimit) external onlyOwner {
     ...
     */
    function setOracleLatestAnswer(address token, int256 latestAnswer) external onlyOwner {
        OracleLatestAnswerInfo storage _oracleLatestAnswerInfo = oracleLatestAnswerInfo[token];
     ...
     * @param token address of token
     */
    function deposit(uint256 amount, address token) external payable onlyUsedToken(token) whenNotPaused {
     ...
     * @param token Address of token
     */
    function withdraw(uint256 amount, address token) external onlyUsedToken(token) whenNotPaused {
        ...
        if (amountExp18 <= tokenBalance[token]) {
            ...
            requireReturnAmount = amount - (tokenBalance[token] / (10 ** (18 - decimals)));
        ...
        if (depositor.depositIterate[token] == countEarns_) {
            depositor.tokenTime[token] -= (block.timestamp * (amountExp18)).toInt256();
        } else {
            depositor.tokenTime[token] =
                (depositor.amount[token] * earnBLID[countEarns_ - 1].timestamp).toInt256() -
        ...
        if (isEnoughBalance) {
            ...
            uint256 requiredLeaveOld = _calcRequiredTokenLeave(token, tokenDeposited[token] + amountExp18);
            uint256 requiredLeaveNow = _calcRequiredTokenLeave(token, tokenDeposited[token]);
            ...
            if (requiredLeaveNow > (tokenBalance[token])) {
                ...
                if (requiredLeaveOld <= requiredLeaveNow + amountExp18) {
                    ...
                    IMultiLogicProxy(logicContract).setLogicTokenAvailable(
                        amount - (requiredLeaveOld - requiredLeaveNow) / 10 ** (18 - decimals),
                        token,
                        0
                    );
                } else {
                    ...
                    IMultiLogicProxy(logicContract).setLogicTokenAvailable(
                        (requiredLeaveOld - requiredLeaveNow) / 10 ** (18 - decimals) - amount,
                        token,
                        1
                    );
                }
            ...
            IMultiLogicProxy(logicContract).releaseToken(requireReturnAmount, token);
     ...
     * @param token Address of token
     */
    function takeToken(uint256 amount, address token) external onlyMultiLogicProxy onlyUsedToken(token) {
        uint8 decimals = _getTokenDecimals(token);
     ...
     * @param token Address of token
     */
    function returnToken(uint256 amount, address token) external onlyMultiLogicProxy onlyUsedToken(token) {
        uint8 decimals = _getTokenDecimals(token);
    ...
    function addEarn(uint256 amount) external onlyMultiLogicProxy {
        ...
        for (uint256 i = 0; i < countTokens_; ) {
            ...
            int256 deltaTime = (block.timestamp - _oracleLatestAnswerInfo.timestamp).toInt256();
            ...
            if (deltaTime > 1200 || uint256(delta) >= (oracleDeviationLimit * 1200)) {
                require(uint256(delta / deltaTime) <= oracleDeviationLimit, "E19");
            }
            ...
            thisEarnBLID.rates[token] = (uint256(latestAnswer) * 10 ** (18 - oracle.decimals()));
            ...
            _dollarTime += tokenTime[token] * thisEarnBLID.rates[token].toInt256();
        ...
        thisEarnBLID.tdt = uint256(
            ((block.timestamp * thisEarnBLID.usd).toInt256() - _dollarTime) / (1 ether)
        ); // count delta of current token time and all user token time

        for (uint256 i = 0; i < countTokens_; ) {
            address token = tokens[i];
            tokenTime[token] = (tokenDeposited[token] * block.timestamp).toInt256(); // count curent token time
    ...
    function balanceEarnBLID(address account) public view returns (uint256) {
        ...
        for (uint256 j = 0; j < countTokens_; ) {
            ...
            if (depositorIterate == depositor.depositIterate[token]) {
                sum += _getEarnedInOneDepositedIterate(depositorIterate, token, account);
                sum += _getEarnedInOneNotDepositedIterate(depositorIterate, token, account);
            } else {
                sum += _getEarnedInOneNotDepositedIterate(depositorIterate - 1, token, account);
            }
     ...
     */
    function getEarnsByID(uint256 id) external view returns (uint256, uint256, uint256) {
     ...
     */
    function getLeaveTokenPolicy() external view returns (LeaveTokenPolicy memory) {
            ...
            _accBLIDpershare =
                ...
                (
                    activeSupplyBLID <= maxActiveBLID
                        ...
                        : ((passedBlockCount * blidPerBlock * maxActiveBLID) / activeSupplyBLID)
                );
        }
     ...
     * @param accountAddress Address of depositor
     */
    function _depositInternal(uint256 amount, address token, address accountAddress) internal {
        ...
        // Calculate requiredLeave and set available for strategy
        uint256 requiredLeaveOld = _calcRequiredTokenLeave(token, tokenDepositedOld);

        uint256 requiredLeaveNow = _calcRequiredTokenLeave(token, tokenDepositedOld + amountExp18);
        ...
        if (requiredLeaveNow > (tokenBalanceOld + amountExp18)) {
            ...
            if (tokenBalanceOld >= requiredLeaveOld) {
                ...
                if (amountExp18 >= (requiredLeaveNow - requiredLeaveOld)) {
                    IMultiLogicProxy(logicContract).setLogicTokenAvailable(
                        amount - (requiredLeaveNow - requiredLeaveOld) / 10 ** (18 - decimals),
                        token,
                        1
                    );
                } else {
                    IMultiLogicProxy(logicContract).setLogicTokenAvailable(
                        (requiredLeaveNow - requiredLeaveOld) / 10 ** (18 - decimals) - amount,
                        token,
                        0
                    );
                }
                ...
                if (amountExp18 >= (requiredLeaveNow - tokenBalanceOld)) {
                    IMultiLogicProxy(logicContract).setLogicTokenAvailable(
                        amount - (requiredLeaveNow - tokenBalanceOld) / 10 ** (18 - decimals),
                        token,
                        1
                    );
                } else {
                    IMultiLogicProxy(logicContract).setLogicTokenAvailable(
                        (requiredLeaveNow - tokenBalanceOld) / 10 ** (18 - decimals) - amount,
                        token,
                        0
                    );
                }
     ...
     */
    function _updateAccumulatedRewardsPerShareById(address token, uint256 id) private {
    ...
    function _getEarnedInOneDepositedIterate(
        ...
        return
            ...
            thisEarnBLID.allBLID *
                ...
                uint256(
                    (thisDepositor.amount[token] * thisEarnBLID.rates[token] * thisEarnBLID.timestamp)
                        .toInt256() - thisDepositor.tokenTime[token] * thisEarnBLID.rates[token].toInt256()
                )) /
    ...
    function _boostingAdjustAmount(
        ...
        // Update activeSupply
        activeSupplyBLID = activeSupplyBLID + userBoost.blidDeposit - oldBlidDeposit;
    ...
    function _getEarnedInOneNotDepositedIterate(
        ...
        mapping(uint256 => uint256) storage accumulatedRewardsPerShareForToken = accumulatedRewardsPerShare[
            token
        ];
        return
            ((accumulatedRewardsPerShareForToken[countEarns - 1] -
                accumulatedRewardsPerShareForToken[depositIterate]) * deposits[account].amount[token]) /
            (1 ether);
    }
     ...
     */
    function _calcUSDPrice(address token, uint256 amountExp18) private view returns (uint256) {
        AggregatorV3Interface oracle = AggregatorV3Interface(oracles[token]);
        (, int256 latestAnswer, , , ) = oracle.latestRoundData();
        return ((amountExp18 * uint256(latestAnswer) * 10 ** (18 - oracle.decimals())) / (1 ether));
    }
    ...
    function _calcRequiredTokenLeave(
        ...
        uint256 limit = (leaveTokenPolicy.limit * 10 ** (decimals) * (1 ether)) / uint256(latestAnswer);

        // 0 - limit : return leavePerentage %
        if (depositTotalExp18 <= limit) return (depositTotalExp18 * leaveTokenPolicy.leavePercentage) / 10000;

        // > limit : return leaveFixed
        return (leaveTokenPolicy.leaveFixed * 10 ** (decimals) * (1 ether)) / uint256(latestAnswer);
    }
