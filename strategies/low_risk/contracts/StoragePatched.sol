...
contract StorageV21Patched is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    /**
     * @notice Get epoch tdt
     * @param _epoch epoch id
     * @param _token address of the token
     */
    function getEpochInfo(uint256 _epoch, address _token) public view returns (uint256, uint256, uint256, uint256) {
        EarnBLID storage thisEarnBLID = earnBLID[_epoch];
        return (
            thisEarnBLID.tdt,
            thisEarnBLID.rates[_token],
     ...
     */
    function getUserTDT(address _user, address _token) public view returns (int256) {
        EarnBLID storage thisEarnBLID = earnBLID[countEarns - 1];
        DepositStruct storage thisDepositor = deposits[_user];
        return ((thisDepositor.amount[_token] * thisEarnBLID.rates[_token] * thisEarnBLID.timestamp).toInt256() -
                thisDepositor.tokenTime[_token] * thisEarnBLID.rates[_token].toInt256()) / (1 ether);
    }

    /**
     * @notice Get token time
     * @param _token address of the token
     */
    function getTokenTime(address _token) public view returns (int256) {
        return tokenTime[_token];
    }

    /**
     * @notice Get user token time
     * @param _user user address
     */
    function getUserTokenTime(address _user, address _token) public view returns (int256) {
        return deposits[_user].tokenTime[_token];
    }
    ...
    function addEarn(uint256 amount) external isLogicContract(msg.sender) {
        ...
        console.log("_____dollarTime");
        console.logInt(_dollarTime);
