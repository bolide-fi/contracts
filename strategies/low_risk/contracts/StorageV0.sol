...
contract StorageV0 is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function deposit(uint256 amount, address token) external isUsedToken(token) whenNotPaused {
        ...
        if (deposits[msg.sender].tokenTime[address(0)] == 0) {
            ...
            deposits[msg.sender].tokenTime[token] += int256(block.timestamp * (amount * 10**(18 - decimals)));
        } else {
            interestFee();
        ...
        tokenTime[token] += int256(block.timestamp * (amount * 10**(18 - decimals)));
    ...
    function withdraw(uint256 amount, address token) external isUsedToken(token) whenNotPaused {
        ...
        if (deposits[msg.sender].depositIterate[token] == countEarns) {
            ...
            deposits[msg.sender].tokenTime[token] =
                int256(deposits[msg.sender].amount[token] * earnBLID[countEarns - 1].timestamp) -
                int256(block.timestamp * (amount * 10**(18 - decimals)));
            deposits[msg.sender].amount[token] -= amount * 10**(18 - decimals);
            deposits[msg.sender].depositIterate[token] = countEarns;
        }
    }
    ...
    function addEarn(uint256 amount) external isLogicContract(msg.sender) {
        ...
        for (uint256 i = 0; i < countTokens; i++) {
            earnBLID[countEarns].rates[tokens[i]] = (uint256(AggregatorV3Interface(oracles[tokens[i]]).latestAnswer()) *
            ...
            _dollarTime += (tokenTime[tokens[i]] * int256(earnBLID[countEarns].rates[tokens[i]])) / int256(1 ether);
        }
        require(_dollarTime != 0);
        earnBLID[countEarns].allBLID = amount;
        earnBLID[countEarns].timestamp = block.timestamp;
        earnBLID[countEarns].tdt = uint256(
            (int256(((block.timestamp) * earnBLID[countEarns].usd) / (1 ether)) - _dollarTime)
        );
        for (uint256 i = 0; i < countTokens; i++) {
            tokenTime[tokens[i]] = int256(tokenDeposited[tokens[i]] * block.timestamp);
        }
    ...
    // external function
    function _upBalance(address account) public {
        for (uint256 i = deposits[account].iterate; i < countEarns; i++) {
            for (uint256 j = 0; j < countTokens; j++) {
                if (i == deposits[account].depositIterate[tokens[j]]) {
                    deposits[account].balanceBLID +=
                        (earnBLID[i].allBLID *
                            uint256(
                                (int256(
                                    (deposits[account].amount[tokens[j]] * earnBLID[i].rates[tokens[j]]) *
                                        earnBLID[i].timestamp
                                ) - (deposits[account].tokenTime[tokens[j]] * int256(earnBLID[i].rates[tokens[j]])))
                            )) /
                        earnBLID[i].tdt /
                        (1 ether);
                } else {
                    deposits[account].balanceBLID +=
                        (earnBLID[i].allBLID *
                            (earnBLID[i].timestamp - earnBLID[i - 1].timestamp) *
                            deposits[account].amount[tokens[j]] *
                            earnBLID[i].rates[tokens[j]]) /
                        earnBLID[i].tdt /
                        (1 ether);
                }
            }
    ...
    function _upBalanceByItarate(address account, uint256 iterate) public {
    ...
    function getBLIDReserve() public view returns (uint256) {
        return reserveBLID;
    }
    ...
    function getTokenDeposit(address account, address token) public view returns (uint256) {
        return deposits[account].amount[token];
    }

    function _isUsedToken(address _token) public view returns (bool) {
        return tokensAdd[_token];
    }

    function getCountEarns() public view returns (uint256) {
        return countEarns;
    }
    ...
    function getTokenDeposited(address token) public view returns (uint256) {
