...
export const storage_boosting = () => {
  ...
  describe("StorageV3", async () => {
    let storageV3: StorageV3, multiLogicProxy: MultiLogic;

    const calcDepositBLIDAmount = async (
      address: string,
      maxBlidPerUSD: BigNumber
    ): Promise<BigNumber> => {
      const userDepositAmount = (await storageV3.balanceOf(address))
        .mul(maxBlidPerUSD)
        .div(ethers.utils.parseEther("1"));

      const userBLIDBalance = await storageV3.getBoostingBLIDAmount(address);
    ...
    before(async () => {
      const StorageV3 = await ethers.getContractFactory("StorageV3", owner);
      const MultiLogicProxy = await ethers.getContractFactory(
        "MultiLogic",
        owner
      );

      storageV3 = (await upgrades.deployProxy(StorageV3, [], {
        initializer: "initialize",
        unsafeAllow: ["constructor"],
      })) as StorageV3;
    ...
    before(async () => {
      let tx = await storageV3.connect(owner).setBLID(blid.address);
      await tx.wait(1);
      ...
      await storageV3.connect(owner).addToken(usdc.address, aggregator.address);

      tx = await storageV3
        .connect(owner)
        .setMultiLogicProxy(multiLogicProxy.address);
      await tx.wait(1);
      await storageV3.connect(owner).setBoostingAddress(expenseer.address);

      tx = await multiLogicProxy.setStorage(storageV3.address);
      await tx.wait(1);
      tx = await aggregator3
        .connect(owner)
        .updateRate("8", firstUSDRate.toString());
      await tx.wait(1);
    });
    ...
    before(async () => {
      let tx = await usdt
        .connect(other1)
        .approve(storageV3.address, ethers.utils.parseEther("10000000000"));
    ...
    describe("setBoostingInfo", async () => {
      ...
      it("set boosting info", async () => {
        const tx = await storageV3
          .connect(owner)
          .setBoostingInfo(MaxBlidPerUSD, BlidPerBlock, MaxActiveBLID);

        const maxBlidPerUSD = await storageV3.connect(owner).maxBlidPerUSD();
        const blidPerBlock = await storageV3.connect(owner).blidPerBlock();
        const maxActiveBLID = await storageV3.connect(owner).maxActiveBLID();
    ...
    describe("depositBLID", async () => {
      it("user must deposit stablecoin before deposit BLID", async () => {
        const tx = await expect(
          storageV3.connect(other1).depositBLID(amountUSDTDeposit)
        ).to.be.revertedWith("E11");
      });
      ...
      it("user deposit BLID for boosting", async () => {
        ...
        await storageV3.connect(other1).depositBLID(depositAmount);

        user1DepositAmount = (await storageV3.balanceOf(other1.address))
          .mul(MaxBlidPerUSD)
          .div(ethers.utils.parseEther("1"));
    ...
    describe("get Claimable Amount", async () => {
      ...
      it("get boosting claimable BLID after one block", async () => {
        const claimableAmount = await storageV3.getBoostingClaimableBLID(
          other1.address
        );
        const blockCount =
          (await ethers.provider.getBlockNumber()) - startBlockUser1 + 1;
    ...
    describe("second deposit", async () => {
      ...
      it("second user deposit BLID", async () => {
        user2DepositAmount = (await storageV3.balanceOf(other2.address))
          .mul(MaxBlidPerUSD)
          .div(3)
          .div(ethers.utils.parseEther("1"));
    ...
    describe("getClaimableBLID after second deposit", async () => {
      ...
      it("get claimable BLID for user1, user2 after 1 blocks", async () => {
        const claimableAmount1 = await storageV3.getBoostingClaimableBLID(
          other1.address
        );
        const claimableAmount2 = await storageV3.getBoostingClaimableBLID(
          other2.address
        );
        const blockCountUser1 =
          (await ethers.provider.getBlockNumber()) - startBlockUser1 + 1;
    ...
    describe("update blidperblock", async () => {
      it("update boosting info", async () => {
        ...
        const blidperBlock = await storageV3.blidPerBlock();
        expect(blidperBlock).to.be.equal(
          secondBlidPerBlock,
          "BlidPerBlock does not udpated"
        );
    ...
    describe("claim reward BLID", async () => {
      before(async () => {
        await blid
          .connect(expenseer)
          .approve(storageV3.address, ethers.utils.parseEther("100000"));
      ...
      it("claim BLID for user1", async () => {
        const beforeBlidbalance = await blid.balanceOf(other1.address);
        const claimableBlid = await storageV3.getBoostingClaimableBLID(
          other1.address
        );

        const tx = await storageV3.connect(other1).claimBoostingRewardBLID();
      ...
      it("claim BLID for user2", async () => {
        ...
        const tx = await storageV3.connect(other2).claimBoostingRewardBLID();
    ...
    describe("Change MaxBLIDPerUSD", async () => {
      ...
      it("update boosting info", async () => {
        ...
        const _maxBlidPerUSD = await storageV3.maxBlidPerUSD();
        expect(_maxBlidPerUSD).to.be.equal(
          secondMaxBlidPerUSD,
          "MaxBlidPerUSD does not udpated"
        );
    ...
    describe("first withdraw", async () => {
      ...
      it("User can't withdraw over balance", async () => {
        ...
        await expect(
          storageV3.connect(other1).withdrawBLID(withdrawAmount)
        ).to.be.revertedWith("E12");
      });
      ...
      it("Withdraw of user 1 for OverDepositPerUSD, claimReward will be using MaxBlidPerUSD", async () => {
        ...
        const claimableBLIDAmount = user1DepositAmount
          ...
          .div(ethers.utils.parseEther("1"));
        const claimableBLIDAmountStorage =
          await storageV3.getBoostingClaimableBLID(other1.address);

        const tx = await storageV3.connect(other1).withdrawBLID(withdrawAmount);
      ...
      it("Withdraw of user 2 for 10%, claimReward will be using MaxBlidPerUSD", async () => {
        ...
        const withdrawAmount = (await storageV3.balanceOf(other2.address))
          .mul(MaxBlidPerUSD)
          .div(10)
          .div(ethers.utils.parseEther("1"));
        const beforeBlidbalance = await blid.balanceOf(other2.address);
        ...
        const claimableBLIDAmount = user2DepositAmount
          ...
          .div(ethers.utils.parseEther("1"));
        const claimableBLIDAmountStorage =
          await storageV3.getBoostingClaimableBLID(other2.address);

        const tx = await storageV3.connect(other2).withdrawBLID(withdrawAmount);
    ...
    describe("Change USD Rate", async () => {
      ...
      it("Update USD Rate", async () => {
        let depositUser1: BigNumber = await storageV3.balanceOf(other1.address);
        let depositUser2: BigNumber = await storageV3.balanceOf(other2.address);
        ...
        tx = await storageV3.setOracleLatestAnswer(
          usdt.address,
          secondUSDRate.toString()
        );
        await tx.wait(1);

        let depositUser1New: BigNumber = await storageV3.balanceOf(
          other1.address
        );
        let depositUser2New: BigNumber = await storageV3.balanceOf(
          other2.address
        );
    ...
    describe("third deposit", async () => {
      ...
      it("deposit of user 1, claimReward will be using secondMaxBlidPerUSD", async () => {
        ...
        const tx = await storageV3.connect(other1).depositBLID(depositAmount);
    ...
    describe("withdraw total", async () => {
      ...
      it("withdraw of user 1", async () => {
        const withdrawAmount = await storageV3.getBoostingBLIDAmount(
          other1.address
        );
        const beforeBlidbalance = await blid.balanceOf(other1.address);
        ...
        await storageV3.connect(other1).withdrawBLID(withdrawAmount);
        ...
        expect(
          (await storageV3.getBoostingBLIDAmount(other1.address)).toString()
        ).to.be.equal("0");
      ...
      it("withdraw of user 2", async () => {
        ...
        const claimableBLIDAmount = await storageV3.getBoostingClaimableBLID(
          other2.address
        );

        await storageV3.connect(other2).withdrawBLID(withdrawAmount);
        ...
        expect(
          (await storageV3.getBoostingBLIDAmount(other2.address)).toString()
        ).to.be.equal("0");
    ...
    describe("Total BLID supply", async () => {
      ...
      it("after first BLID deposit for boosting", async () => {
        const beforeTotalBlidSupply = await storageV3.totalSupplyBLID();
        ...
        const afterTotalBlidSupply = await storageV3.totalSupplyBLID();
        expect(afterTotalBlidSupply).to.be.equal(
          beforeTotalBlidSupply.add(depositAmount),
          "Total BLID supply should be updated after BLID deposit"
        );
      ...
      it("after second BLID deposit for boosting", async () => {
        ...
        await storageV3.connect(other2).depositBLID(depositAmount);
      ...
      it("after claim BLID for user1", async () => {
        ...
        await storageV3.connect(other1).claimBoostingRewardBLID();
      ...
      it("after claim BLID for user2", async () => {
        ...
        await storageV3.connect(other2).claimBoostingRewardBLID();
    ...
    describe("MaxActiveBLID", async () => {
      let USDTBalanceUser1: BigNumber;
      before(async () => {
        USDTBalanceUser1 = await storageV3.balanceOf(other1.address);
      });
    ...
    describe("Time Subtraction Check", async () => {
      ...
      it("Withdraw total USDT", async () => {
        await storageV3
          .connect(other1)
          .withdraw(
            await storageV3.getTokenDeposit(other1.address, usdt.address),
            usdt.address
          );
        ...
        await storageV3
          .connect(other2)
          .withdraw(
            await storageV3.getTokenDeposit(other2.address, usdt.address),
            usdt.address
          );

        expect(
          (await storageV3.getTokenDeposited(usdt.address)).toString()
        ).to.be.equal("0", "USDT in storage should be 0");

        expect(
          (await storageV3.balanceOf(other1.address)).toString()
        ).to.be.equal("0", "User 1 balance should be 0");

        expect(
          (await storageV3.balanceOf(other2.address)).toString()
        ).to.be.equal("0", "User 2 balance should be 0");
      });
      ...
      it("Start new epic", async () => {
        await storageV3
          .connect(owner)
          .setMultiLogicProxy(logicContract.address);
        await storageV3.connect(logicContract).addEarn(_addEarnAmount);
        await storageV3
          .connect(owner)
          .setMultiLogicProxy(multiLogicProxy.address);
      });
      ...
      it("Add Earn 50", async () => {
        ...
        const blidBalanceStorageAfter = await blid.balanceOf(storageV3.address);
      ...
      it("Check of earning", async () => {
        const earned1 = await storageV3.balanceEarnBLID(other1.address);
        const earned2 = await storageV3.balanceEarnBLID(other2.address);
    ...
    describe("Oracle KillSwitch", async () => {
      ...
      it("Add earn should be failed", async () => {
        await storageV3
          ...
          .setMultiLogicProxy(logicContract.address);
        await expect(
          storageV3.connect(logicContract).addEarn("50")
        ).to.be.revertedWith("E19");
      });
      ...
      it("Add earn should be success", async () => {
        await storageV3.connect(logicContract).addEarn("50");
      });
    });
  });
};
