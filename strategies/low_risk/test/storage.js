...
const Storage = artifacts.require("StorageV0");
// const Usdt = cc.fromArtifact("ERC20");
// const Aggregator = cc.fromArtifact("Aggregator");
// const AggregatorN2 = cc.fromArtifact("AggregatorN2");
// const Usdt = artifacts.require("ERC20");
// const Aggregator = artifacts.require("Aggregator");
// const AggregatorN2 = artifacts.require("AggregatorN2");
...
contract('Storage', accounts => {
  ...
  let blid, usdt, storage, aggregator, startTime, truffle, Usdt, Aggregator, AggregatorN2
  ...
  before(async () => {
    ...
    Usdt = truffle.fromArtifact("ERC20");
    Aggregator = truffle.fromArtifact("Aggregator");
    AggregatorN2 = truffle.fromArtifact("AggregatorN2");

    aggregator = await Aggregator.new();

    console.log(await truffle.web3.eth.getBalance(logicContract));
    console.log(222)
    blid = await Usdt.new("some erc20 as if BLID", "SERC", {from: logicContract, gas : 1000000});
    console.log(333)
    usdt = await Usdt.new("some erc20", "SERC", {from: owner, gas : 1000000});
    // storage = await Storage.new(logicContract, {from: owner1})
...
it('deploys storage successfully', async () => {
  ...
  describe('standart scence', async () => {
    ...
    it('withdraw when amount equal zero', async () => {
      await storage.deposit("0",usdt.address, {from:Dmitry}).should.be.rejectedWith("Returned error: VM Exception while processing transaction: revert Amount need more than zero -- Reason given: Amount need more than zero.")
    })
  ...
  describe('new token', async () => {
    ...
    before(async () => {
      usdt = await Usdt.new("some erc20","SERC", {from: owner, gas : 1000000})
      usdtn2 = await Usdt.new("some erc20","SERC", {from: owner, gas : 1000000})
      storage = await Storage.deployed();
      await storage.setBLID(blid.address, {from: owner})
    })
    before(async () => {
  ...
  describe('deposit all withdraw addEarn interestFee', async () => {
    ...
    it('first add earn', async () => {
      ...
      assert.
      equal(balance.toString(),"666666666666");
    })
  })
  ...
  describe('many deposit', async () => {
    before(async () => {
      ...
      blid = await Usdt.new("some erc20 as if BLID","SERC", {from: logicContract, gas : 1000000})
      usdt = await Usdt.new("some erc20","SERC", {from: owner, gas : 1000000})
      usdc = await Usdt.new("some erc20","SERC", {from: owner, gas : 1000000})
      storage = await Storage.deployed();
      await storage.setBLID(blid.address, {from: owner})
    })
    before(async () => {
